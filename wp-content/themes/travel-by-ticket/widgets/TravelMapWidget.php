...
class TravelMapWidget extends \Elementor\Widget_Base {
    ...
    public function get_style_depends() { return ['bus-stations-map']; }
    public function get_script_depends() { return ['bus-stations-map']; }
    ...
    protected function render() {
        ...
        ?>
        <div id="<?php echo esc_attr($map_id); ?>" class="tw-bus-map" style="height: 520px;"></div>
        ...
        (function(){
            ...
            const lang = (pageLang === 'en') ? 'en' : 'bs-BA';

            const map = L.map(<?php echo json_encode($map_id); ?>, { preferCanvas: true });
            ...
            const label = (lang === 'en') ? 'Bus station' : 'Autobuska stanica';

            const manualStations = {
                "Podgorica": [
                    { name: `${label} Podgorica`, lat: 42.4329, lon: 19.2597 }
                ],
                "Kotor": [
                    { name: `${label} Kotor`, lat: 42.4247, lon: 18.7682 }
                ],
                "Budva": [
                    { name: `${label} Budva`, lat: 42.2876, lon: 18.8410 }
                ],
                "Tivat": [
                    { name: `${label} Tivat`, lat: 42.4323, lon: 18.7068 }
                ],
                "Žabljak": [
                    { name: `${label} Žabljak`, lat: 43.1544, lon: 19.1235 }
                ],
                "Nikšić": [
                    { name: `${label} Nikšić`, lat: 42.7734, lon: 18.9445 }
                ]
            };

            Object.entries(manualStations).forEach(([city, arr]) => {
                const group = L.layerGroup(
                    arr.map(s => L.marker([s.lat, s.lon], { icon: busIcon })
                                   .bindPopup(`<b>${s.name}</b><br/>🏙️ ${city}`))
                );
                overlays[city] = group; group.addTo(map);
                layersControl.addOverlay(group, `🚌 ${city} (${arr.length})`);
            });
